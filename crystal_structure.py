...
def print_atomic_info(atoms, scale_factor=1.0):
    """
    ...
    print("-" * 60)
    print("Atom\tX\t\tY\t\tZ\t\tRadius (Å)")
    print("-" * 60)
    
    for atom in atoms:
        element = ''.join(filter(str.isalpha, atom['name']))
        radius = get_atomic_radius(element) * scale_factor
        print(f"{atom['name']:6s}\t{atom['x']:8.6f}\t{atom['y']:8.6f}\t{atom['z']:8.6f}\t{radius:8.2f}")
...
def main():
    ...
    print_atomic_info(atoms, args.scale)
